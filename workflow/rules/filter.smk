...
rule feauture_distribution_peak:
    ...
    run:
        ...
        df.loc[:, "five_prime_utr":"transposable_element"] = df.loc[
        :, "five_prime_utr":"transposable_element"
        ].div(df.sum(axis=1), axis=0)
        data = df.sort_index()
        data = data * 100
...
rule plot_polyadenylation_by_feature:
    ...
    run:
        ...
        df.loc[:, "five_prime_utr":"three_prime_utr"] = df.loc[
...
rule plot_premature_polyadenylation_x_expression:
    ...
    run:
        ...
        files = [
            f"{prefix}_{sufix}"
            for prefix in input
            for sufix in ["three_out.txt", "cds_out.txt"]
        ]
        ...
        for file in files:
            ...
            if "cds_out" in file:
                df_tmp = pd.read_csv(
                    file, sep="\t", header=None, index_col=0
                ).sort_index()
                if not df_cds.empty:
...
rule plot_polyadenylation_in_cds_by_region:
    ...
    run:
        ...
        labels = [
            str(int((int(x) / len(data.columns)) * 100)) + "%" for x in data.columns
        ]
        x_pos = np.arange(len(labels))
        means = list(data.mean())
        stds = list(data.std())
    ...
    run:
        ...
        for cond in cond_dict_in_frame.keys():
            for i in range(len(cond_dict_in_frame[cond])):
                ...
                cond_dict_in_frame[cond][i] = (
                    cond_dict_in_frame[cond][i] / total
                ) * 100
                cond_dict_out_frame[cond][i] = (
                    cond_dict_out_frame[cond][i] / total
                ) * 100
        ...
        ax.set_xticklabels(labels)
        ax.set_title(
        "Premature Polyadenylation without STOP Codon in frame in different conditions"
        )
    ...
    run:
        ...
        for i, c in enumerate(conditions_labels):
            if i == 0:
                ...
                statistic, pvalue = stats.ttest_ind(
                    data[conditions_labels[0]], data[conditions_labels[i]]
                )
                print(
        ...
        for p, sig in zip(ax.patches, significance_list):
            if sig == True:
                ax.annotate(
                    "*",
                    (p.get_x() + p.get_width() / 2.0, p.get_height() + 0.7),
                    ha="center",
                    va="center",
                    fontsize=11,
                    color="black",
