    ...
    for step in step_set:
        ...
        if step.type == 'CDS':
            ...
            if read.iv.strand == '+':
                ...
                if ((read.iv.start - step.iv.start) % 3 == 1 and read.read.seq.decode("utf-8")[-1] == 'T') or ((read.iv.start - step.iv.start) % 3 == 2 and read.read.seq.decode("utf-8")[-2:] == 'TG'):
                ...
                if ((step.iv.start - read.iv.start) % 3 == 1 and read.read.seq.decode("utf-8")[-1] == 'T') or ((step.iv.start - read.iv.start) % 3 == 2 and read.read.seq.decode("utf-8")[-2:] == 'TG'):
                    if 'soft-clipped' not in str(read.cigar):
                        #print(read)
                        #print(read.read)
                        stop_in_frame['in_frame'] += 1
