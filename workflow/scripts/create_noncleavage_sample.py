...
def subtract_bed(coverage, bed_list, chrom_len, region_len):
    ...
    chrom_dict = {
        chrom: {strand: list() for strand in ["+", "-"]} for chrom in chrom_len.index
    }
    ...
    for chrom in chrom_len.index:
        ...
        for strand in ["+", "-"]:
            ...
            while i < chrom_len.loc[chrom].length - region_len:
                ...
                region_sum = sum(
                    coverage.chrom_vectors[chrom][strand][i : i + region_len]
                )
    ...
    with open(output, "w") as fout:
        for i in range(int(number_regions)):
            ...
            if chrom_dict[chrom][strand]:
                ...
                fout.write(
                    f"{chrom}\t{pos}\t{(pos+region_len)}\tregion_{i}\t1000\t{strand}\n"
                )
