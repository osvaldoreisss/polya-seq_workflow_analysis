...
def get_distribution(genome, sam):
    ...
    for line in bam_reader:
        ...
        fasta = pysam.faidx(sequence_iterator.fos, "%s:%d-%d" % (read.chrom, read.start, read.end - 1))
    ...
    df_tmp = pd.DataFrame(np.array([list(distr_as.values())]),index=[sam], columns=distr_as.keys()) 
    return df_tmp
...
def get_random_transcript_position(transcripts):
    ...
    for key in distr_as.keys():
        distr_as[key]=(distr_as[key]/num_seqs)*100
    df_tmp = pd.DataFrame(np.array([list(distr_as.values())]),index=['random'], columns=distr_as.keys()) 
    return df_tmp
