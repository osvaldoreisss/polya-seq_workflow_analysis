    ...
    for feature in gtf:
        ...
        if feature.type == "stop_codon":
            ...
            if three_utr_feature.iv.strand == "+":
                ...
                three_utr_feature.iv.start = (
                    three_utr_feature.iv.start - three_length - 1
                )
...
def feature_classify(iv, gaos):
    ...
    feature_genes = set(
        re.findall(
            r"(\w+[prime|CDS|ncRNA|rRNA|tRNA|snoRNA|snRNA|pseudogene|transposable_element]\w+\s'\w+[-|(]*\w+[)]*\w*')",
            str(step_set),
        )
    )
...
def main():
    ...
    for gene in gene_set:
        ...
        try:
            ...
            pseudogene_out.write(f"{gene}\t{genes['pseudogene'][gene]}\n")
        try:
            transposable_element_out.write(
                f"{gene}\t{genes['transposable_element'][gene]}\n"
            )
        except (KeyError, TypeError) as _:
