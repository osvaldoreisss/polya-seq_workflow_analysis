...
def peak_detection(sam, chrom_len, output_bam):
    ...
    for chrom in chrom_len.index:
        for strand in ["+", "-"]:
            chrom_array[chrom].append(
                (
                    find_peaks(
                        np.array(
                            [
                                coverage.chrom_vectors[chrom][strand][i]
        ...
        for s in range(0, 2):
            ...
            for i in range(0, len(peaks)):
                ...
                if feature:
                    ...
                    peaks_dict[(chrom, strand, gene, feature, peaks[i])] = peak_heights[
                        i
                    ]
    return peaks_dict
...
def write_to_bed(peaks_dict, bedfile, region_len):
    ...
    with open(bedfile, "w") as fout:
        for key in peaks_dict.keys():
            ...
            fout.write(
                f"{chrom}\t{(peak-region_len)}\t{(peak+region_len)}\tpeak|{gene}|{feature}|{i}\t{peaks_dict[key]}\t{strand}\n"
            )
            i += 1
...
def write_bed_by_feature(peaks_dict, bedfile, feature_name, region_len):
    ...
    with open(bedfile, "w") as fout:
        for key in peaks_dict.keys():
            (chrom, strand, gene, feature, peak) = key
            if feature == feature_name:
                relative_peak_expression = float(
                    int(peaks_dict[key]) / gene_expression[gene]
                )
                fout.write(
                    f"{chrom}\t{(peak-region_len)}\t{(peak+region_len)}\tpeak|{gene}|{feature}|{i}\t{relative_peak_expression}\t{strand}\n"
                )
            i += 1
...
def main():
    ...
    write_bed_by_feature(
        peaks_dict, args.output_three, "three_prime_utr", int(args.region_len)
    )
